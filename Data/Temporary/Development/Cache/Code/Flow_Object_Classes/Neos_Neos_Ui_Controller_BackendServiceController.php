...
class BackendServiceController extends BackendServiceController_Original implements \Neos\Flow\ObjectManagement\Proxy\ProxyInterface {
    ...
    protected function Flow_Aop_Proxy_buildMethodsAndAdvicesArray()
    {
        ...
        $this->Flow_Aop_Proxy_targetMethodsAndGroupedAdvices = array(
            ...
            'generateUriPathSegmentAction' => array(
                'Neos\Flow\Aop\Advice\AroundAdvice' => array(
                    new \Neos\Flow\Aop\Advice\AroundAdvice('Neos\Flow\Security\Aspect\PolicyEnforcementAspect', 'enforcePolicy', $objectManager, NULL),
                ),
            ),
     ...
     * @throws \Neos\Neos\Exception
     */
    public function generateUriPathSegmentAction(\Neos\ContentRepository\Domain\Model\NodeInterface $contextNode, string $text) : void
    {

        if (isset($this->Flow_Aop_Proxy_methodIsInAdviceMode['generateUriPathSegmentAction'])) {
            parent::generateUriPathSegmentAction($contextNode, $text);

        } else {
            $this->Flow_Aop_Proxy_methodIsInAdviceMode['generateUriPathSegmentAction'] = true;
            try {
            
                $methodArguments = [];

                $methodArguments['contextNode'] = $contextNode;
                $methodArguments['text'] = $text;
            
                $adviceChains = $this->Flow_Aop_Proxy_getAdviceChains('generateUriPathSegmentAction');
                $adviceChain = $adviceChains['Neos\Flow\Aop\Advice\AroundAdvice'];
                $adviceChain->rewind();
                $joinPoint = new \Neos\Flow\Aop\JoinPoint($this, 'Neos\Neos\Ui\Controller\BackendServiceController', 'generateUriPathSegmentAction', $methodArguments, $adviceChain);
                $result = $adviceChain->proceed($joinPoint);
                $methodArguments = $joinPoint->getMethodArguments();

            } catch (\Exception $exception) {
                unset($this->Flow_Aop_Proxy_methodIsInAdviceMode['generateUriPathSegmentAction']);
                throw $exception;
            }
            unset($this->Flow_Aop_Proxy_methodIsInAdviceMode['generateUriPathSegmentAction']);
        }
  ...
  16 => 'translator',
  17 => 'nodeUriPathSegmentGenerator',
  18 => 'objectManager',
  19 => 'reflectionService',
  20 => 'mvcPropertyMappingConfigurationService',
  21 => 'viewConfigurationManager',
  22 => 'validatorResolver',
  23 => 'defaultViewImplementation',
);
    }
}
# PathAndFilename: /var/www/neos/Packages/Application/Neos.Neos.Ui/Classes/Controller/BackendServiceController.php
#